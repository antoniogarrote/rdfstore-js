...
TurtleParser.parser.parse = function(data, graph) {
    ...
    for(var i=0; i<result.length; i++) {
        var unit = result[i];
        if(unit.token === 'base') {
            namespaces[''] = unit.value;
            namespaces['base'] = unit.value;
            ...
            for(var j=0; j<unit.triplesContext.length; j++) {
                ...
                var quad = { subject: TurtleParser.parser.normalizeTerm(triple.subject, namespaces),
                             predicate: TurtleParser.parser.normalizeTerm(triple.predicate, namespaces),
                             object: TurtleParser.parser.normalizeTerm(triple.object, namespaces),
                             graph: graph };
...
TurtleParser.parser.normalizeLiteral = function(term, ns) {
    ...
    if(value != null && type != null && typeof(type) != 'string') {
        ...
        if(typeValue != null) {
            ...
            var resolvedPrefix = ns[typePrefix];
...
TurtleParser.parser.normalizeBaseUri = function(term, ns) {
    var uri = null;
    if(term.value == null) {
        var prefix = term.prefix;
        var suffix = term.suffix;
        var resolvedPrefix = ns[prefix];
    ...
    if(uri===null) {
        ...
        if(!uri.indexOf(":") != -1) {
            uri = (ns.base||"") + uri; //this.applyBaseUri(uri, env);
        } else {
...
TurtleParser.parser.normalizeTerm = function(term, ns) {
    if(term.token === 'uri') {
        return {'uri': TurtleParser.parser.normalizeBaseUri(term, ns)}
    } else if(term.token === 'literal') {
        return {'literal': TurtleParser.parser.normalizeLiteral(term, ns)};
