...
QueryFilters.runFilter = function(filterExpr, bindings, queryEngine, dataset, env, callback) {
    if(callback === undefined) throw new Error("runFilter is asynchronous");
...
QueryFilters.runComparisonFunction = function(op1, op2, compareFn, dateCompareFn, bindings) {
    ...
    if(QueryFilters.isNumeric(op1) && QueryFilters.isNumeric(op2)) {
        return QueryFilters.ebvBoolean(compareFn(QueryFilters.effectiveTypeValue(op1), QueryFilters.effectiveTypeValue(op2)));
        ...
        var comp = Utils.compareDateComponents(op1.value, op2.value);
        if(comp != null) {
            return QueryFilters.ebvBoolean(dateCompareFn(comp));
        } else {
            return QueryFilters.ebvError();
        }
    } else {
...
QueryFilters.runIriRefOrFunction = function(iriref, args, bindings,queryEngine, dataset, env, callback) {
    if(args == null) {
        callback(iriref);
        return;
    } else {
        var ops = [];
        _.eachSeries(args, function(arg, k) {
            QueryFilters.runFilter(arg, bindings, queryEngine, dataset, env, function(op) {
                ops.push(op);
                k();
            })
        },
        function() {
            callback(syncPartOfRunIRIRefOrFunction(iriref, ops, queryEngine, env));
        });
    }
};

function syncPartOfRunIRIRefOrFunction(iriref, ops, queryEngine, env) {
